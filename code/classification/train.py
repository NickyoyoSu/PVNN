...
def main(args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, (x, y) in tqdm(enumerate(train_loader)):
            # ------- Start iteration -------
            # Ensure input dtype matches model parameters to avoid Float/Double mismatch
            input_dtype = next(model.parameters()).dtype
            x = x.to(device).to(input_dtype)
            y = y.to(device)
            ...
            optimizer.zero_grad()
            loss.backward()
            if hasattr(model.module, 'decoder') and isinstance(model.module.decoder, LorentzMLR):
...
def evaluate(model, dataloader, criterion, device):
    ...
    for i, (x, y) in enumerate(dataloader):
        # Match evaluation input dtype with model parameters
        input_dtype = next(model.parameters()).dtype
        x = x.to(device).to(input_dtype)
        y = y.to(device)
