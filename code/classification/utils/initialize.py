...
class TinyImageNetValDataset(torch.utils.data.Dataset):
    """Tiny-ImageNet validation split that reads labels from val_annotations.txt."""
    def __init__(self, img_dir, anno_file, transform=None):
        self.img_dir = img_dir
        self.transform = transform
        self.img_labels = self._load_annotations(anno_file)
    ...
    def _load_annotations(self, anno_file):
        ...
        with open(anno_file, 'r') as f:
            for line in f:
                ...
                img_name = parts[0]
                label = parts[1]  # class ID such as n01443537
                img_labels.append((img_name, label))
        # Build label-to-index mapping (Tiny-ImageNet has 200 classes)
        unique_labels = sorted(set(label for _, label in img_labels))
        self.label_to_idx = {label: idx for idx, label in enumerate(unique_labels)}
        return [(img, self.label_to_idx[label]) for img, label in img_labels]
...
def select_dataset(args, validation_split=False):
    ...
    # Tiny-ImageNet
        ...
        root_dir = DATA_ROOT / "tiny-imagenet-200"
        train_dir = root_dir / "train"
        val_dir = root_dir / "val" / "images"
        val_anno_file = root_dir / "val" / "val_annotations.txt"
        test_dir = root_dir / "test" / "images"

        if not train_dir.exists():
            raise FileNotFoundError(
                f"Tiny-ImageNet not found. Please place the dataset under '{root_dir}'."
            )
        ...
        train_set = datasets.ImageFolder(str(train_dir), train_transform)
        val_set = TinyImageNetValDataset(str(val_dir), str(val_anno_file), test_transform)
        test_set = val_set
