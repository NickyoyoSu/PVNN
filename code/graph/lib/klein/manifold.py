import torch
from ..lorentz import manifold as lm
import torch.nn as nn
import torch.nn.functional as F
import math  # math
from ..utils.math_ops import tanh, atanh
...
def proj(self, x, c=None):
    ...
    # 
    print(f"\n===== Klein =====")
    print(f": {x.shape}, : {k_val}")
    
    # NaN/Inf
    has_nan = torch.isnan(x).any()
    has_inf = torch.isinf(x).any()
    if has_nan or has_inf:
        print(f":  {'NaN' if has_nan else ''}{'' if has_nan and has_inf else ''}{'Inf' if has_inf else ''}")
    
    # 
    x_norm = torch.norm(x, dim=-1, keepdim=True)
    print(f": ={x_norm.min().item():.6f}, ={x_norm.max().item():.6f}, ={x_norm.mean().item():.6f}")
    
    # /
    very_small_norms = (x_norm < 1e-8).sum().item()
    if very_small_norms > 0:
        print(f": {very_small_norms}")
...
class KleinManifold(torch.nn.Module):
    ...
    def proj(self, x, c=None):
        ...
        if torch.isnan(x_safe).any():
            print("\n--- Klein.proj NaN ---")
            print(f" x shape: {x.shape}, c: {k_val}")
            if torch.isnan(x).any(): print(": Proj  x  NaN")
            if torch.isinf(x).any(): print(": Proj  x  Inf")
            
            print(f" (x_norm): min={x_norm.min():.4f}, max={x_norm.max():.4f}, has_nan={torch.isnan(x_norm).any()}, has_inf={torch.isinf(x_norm).any()}")
            if has_norm_nan_or_inf:
                print("-> NaN/Inf, ")

            print(f": {max_norm:.4f}, : {mask.sum()}")
            print(f" x_safe: has_nan={torch.isnan(x_safe).any()}, has_inf={torch.isinf(x_safe).any()}")
            print("--- Klein.proj  ---\n")
    ...
    def scalar_mul(self, r, x, c=None):
        ...
        return self.proj(result, c=k_val)  # Klein
    ...
    def add(self, x, y, c=None, dim=-1):
        ...
        if torch.isnan(final_result).any():
            print("\n--- Klein.add (mobius_add) NaN ---")
            print(f" x shape: {x.shape}, y shape: {y.shape}, c: {k_val}")
            if torch.isnan(x).any(): print(":  x  NaN")
            if torch.isinf(x).any(): print(":  x  Inf")
            if torch.isnan(y).any(): print(":  y  NaN")
            if torch.isinf(y).any(): print(":  y  Inf")
            ...
            print(f" result: has_nan={torch.isnan(result).any()}, has_inf={torch.isinf(result).any()}")
            print(f" final_result: has_nan={torch.isnan(final_result).any()}")
            print("--- Klein.add (mobius_add)  ---\n")

        return final_result
    
    def mobius_add(self, x, y, c=None, dim=-1):
       """
       Mobius - KleinEinstein
       Einstein
      """
    ...
    def dist(self, x, y, c=None):
        ...
        # acosh
        from ..utils.math_ops import acosh
        sqrt_k = torch.sqrt(torch.tensor(k_val, device=x.device, dtype=x.dtype))
...
class KleinManifoldMLR(nn.Module):
    ...
    (Ganea et al. 2018; Klein formulation  [oai_citation:1arXiv](https://arxiv.org/abs/2410.16813?utm_source=chatgpt.com) [oai_citation:2CVF Open Access](https://openaccess.thecvf.com/content_CVPR_2020/papers/Khrulkov_Hyperbolic_Image_Embeddings_CVPR_2020_paper.pdf?utm_source=chatgpt.com))
    """
    def __init__(self, manifold, in_features, num_classes, bias=True):
        super().__init__()
        self.manifold = manifold          #  proj / logmap0
    ...
    def forward(self, x):
        ...
        # Klein  & 
        x = self.manifold.proj(x, c=k_val)                # (...,d)
        x_tan = self.manifold.logmap0(x, c=k_val)         # c
...
if __name__ == "__main__":
    ...
    print(f": {torch.abs(constraint - target).max().item():.8f}")
    
    # LorentzKlein
    x2 = lorentz_to_klein(X, k)
    
    # 
    print(f": {torch.abs(x - x2).max().item():.8f}")
