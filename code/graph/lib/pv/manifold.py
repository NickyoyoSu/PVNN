...
class PVManifold:
    ...
    def __init__(self, c: float):
        ...
        self.c = float(c)
        self.s = 1.0 / math.sqrt(self.c)   # s = 1/sqrtc
        self.sqrtc = math.sqrt(self.c)     # sqrt(c) used by exp/log maps

    # ----- Exp/Log at the origin -----
    def exp0(self, v: torch.Tensor) -> torch.Tensor:
        # Exp_0(v) = (1/sqrtc) sinh(sqrtc ||v||) v/||v||  == s * sinh(||v||/s) * v/||v||
    ...
    def log0(self, y: torch.Tensor) -> torch.Tensor:
        # Log_0(y) = (1/sqrtc) asinh(sqrtc ||y||) y/||y|| == s * asinh(||y||/s) * y/||y||
    ...
    def pt_0_to_y(self, y: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        b = beta(y, self.c)                                # (...,1)
        coef = self.c * b / (1.0 + b)                      # c * beta/(1+beta)
    ...
    def gyro_add(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        # x oplus y = x + y + { beta_x/(1+beta_x) * <x,y>/s^2 + (1-beta_y)/beta_y } x
        b_x = beta(x, self.c)                               # (...,1)
        b_y = beta(y, self.c)                               # (...,1)
    ...
    def gyro_scalar_mul(self, r: torch.Tensor, x: torch.Tensor, zmax: float = 20.0) -> torch.Tensor:
        """
        PV gyro-scalar multiplication (per screenshot Eq. (3)):
            r otimes_PV x = s * sinh( r * asinh( ||x|| / s ) ) * x / ||x||
        where s = 1/sqrt(c). Broadcasts r to x as needed.
        """
        # make r tensor on same device/dtype; allow scalar or broadcastable shapes
        r = torch.as_tensor(r, device=x.device, dtype=x.dtype)
        # norms
        rx = x.norm(dim=-1, keepdim=True)
        # tau = asinh(||x||/s)
        tau = torch.asinh(rx / self.s)
        # z = r * tau; limit amplitude to avoid overflow in sinh
        z = r * tau
        z = torch.clamp(z, -float(zmax), float(zmax))
        # s * sinh(z) * x / ||x||
        coef = self.s * torch.sinh(z) / rx.clamp_min(_eps(x))
        y = coef * x
        # diagnostics only if abnormal
        if torch.isnan(y).any() or torch.isinf(y).any():
            with torch.no_grad():
                print("[PV.gyro_scalar_mul][diag] output NaN/Inf; ||x|| max=",
                      float(rx.max().item()), " | z max=", float(z.abs().max().item()))
        return y
    ...
    def exp_map(self, x: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        ...
        w = Exp_p(v) ominus p = lambda_x * _sinhc(sqrtc * ||v||_p) * dpi_p[v] [cite: 634, 643]
    ...
    def log_map(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        A simplified reconstruction based on [cite:707]:
        d_pq = dist(p, q)
        sigma = d_pq / ||Z_pv||
        tau_term = (c * beta_p / (1+beta_p)) * sigma * <p, Z_pv>
        Log_p(q) = sigma * Z_pv + tau_term * p
        """
        ...
        # tau_coef = (c * beta_p / (1+beta_p)) * sigma [cite: 707]
...
class PVManifoldMLR(nn.Module):
    ...
    Params per class k:
        z_k in R^d  (origin normal / direction)
        r_k in R    (bias along geodesic in direction z_k)
...
# =====================================================================
#        Optional ambient (w, sigma) head (same semantics; c/s version)
# =====================================================================
