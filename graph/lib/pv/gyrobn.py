...
class GyroBNPV(nn.Module):
    """
    PV-specific GyroBN implementation aligned with the GyroBN_new formulation.

    Key updates:
        1. Correct gyro inverse (gyroinv)
        2. Gyro translations instead of Mbius addition
        3. PV gyro scalar multiplication
    """
    ...
    def frechet_mean(self, x, max_iter=1000):
        ...
        while iters < safety_cap:
            #  log_map(mean, x_i) for all i
            # PV manifold log_map(u, w)
    ...
    # _assign_to_centers/gyro_midpoint_means_multi/grouped_variance_scalar
